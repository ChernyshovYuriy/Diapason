...
- **Spinto Tenor / Dramatic Tenor** and **Lyric Baritone / Kavalierbariton** are genuine acoustic overlaps: a perfect-match profile for the first of each pair scores only 1 point higher (14 vs. 13) on the second than on itself — the closest margin that occurs anywhere in the table (tied with 4 other adjacent pairs at that same 1-point margin). Corrected 2026-09-01: this bullet, and `AdjacentFachDiscriminationTest`'s own docblock, previously claimed both pairs score an *exact* 14/14 tie — true before the ceiling/floor scoring swap (#6 above), but silently no longer true after it; found via direct execution during a later, separate audit pass, not by trusting the existing (still-passing, since its assertion was `>=` not `==`) test.
